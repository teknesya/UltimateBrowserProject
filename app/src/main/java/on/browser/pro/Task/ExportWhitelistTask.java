...
public class ExportWhitelistTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if (result)
            OnBrowserProToast.show(context, context.getString(R.string.toast_export_whitelist_successful) + path);
        else
            OnBrowserProToast.show(context, R.string.toast_export_whitelist_failed);
